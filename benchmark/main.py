...
async def _main(cfg: Config) -> int:
    ...
    live = LiveUI(
        tracker=tracker,
        header=f"Agentic benchmark — {cfg.model} @ {cfg.base_url} (thinking={'on' if cfg.enable_thinking else 'off'}, temp={cfg.temperature}, seed={cfg.seed})",
        enabled=sys.stderr.isatty(),
    )
    live.start()
    try:
        print(
            f"benchmark: {cfg.model} @ {cfg.base_url} concurrency={cfg.concurrency} "
